...
void encode_fun(unsigned char len, unsigned char* key, unsigned char* input, unsigned char* output)
{
    int i = 0, j = 0;
    unsigned char* p = (unsigned char*)malloc(50);      //定义一个50字节缓存区
...
unsigned char Data_plain[32] = { 0xf6, 0x66, 0xae, 0xf4, 0xfe, 0x17, 0xae, 0x18, 0xae, 0x68, 0xc7, 0x81, 0x18, 0x86, 0xc1, 0x9b, 0xf5, 0xea, 0xe6, 0x22, 0x8f, 0x42, 0xcf, 0xf0, 0xd0, 0x15, 0x2a, 0xb7, 0xdc, 0xb3, 0x68, 0x4f };

int exception_swap(LPEXCEPTION_POINTERS p_exinfo)
{
    Data_plain[23] ^= Data_plain[24];
    Data_plain[24] ^= Data_plain[23];
    Data_plain[23] ^= Data_plain[24];
...
int main() {
    unsigned char i, len;
    unsigned char encode_Result[32] = { 0 };
    ...
    for (int i = 0; i <= 512; i++) {
        ...
        __except (exception_swap(GetExceptionInformation())) {
        }
    }
    printf("Give me a 5rn4 plaintext: ");
    scanf_s("%s", input, 33);
    encode_fun(32, key, input, encode_Result);
    if (!memcmp(encode_Result, Data_plain, 32)) {
        printf("PERFECT!\n");
    }
    else {
        printf("MISS!\n");
